...
client.on("ready", () => {
    ...
    client.user.setPresence({
        status: "online",
        activity: {
            name: `Hoạt động tại ${client.guilds.cache.size} guilds`,
            type: "PLAYING",
        },
    });
    /*if (process.env.TYPE_RUN == 'production') {
...
client.on("guildCreate", async newguild => {
    ...
    client.channels.cache.get('743054633747873813').send(embed);
});
...
client.on("message", async message => {
    ...
    if (!serverData) {
        serverData = db.set(message.guild.id, { prefix: "s.", logchannel: null, msgcount: true, defaulttts: null, botdangnoi: false, aiChannel: null, msgChannelOff: [], blacklist: false });
    }
    ...
    if (command) {
        ...
        if (blacklist_status === true && command.name !== 'blacklist') return message.channel.send('Server bạn đang nằm trong blacklist, vui lòng liên hệ owner của bot');
...
y.addListener("data", res => {
    ...
    client.channels.cache.get("743054633747873813").send(send);
});
// end console chat
